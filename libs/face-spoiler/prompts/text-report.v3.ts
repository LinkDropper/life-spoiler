 ...
 *  - 연애운(love): 입·눈·눈썹·광대 — 표정·감정·관계 신호 담당
...
이 섹션은 관계 안에서 드러나는 분위기, 감정의 결, 애정 표현의 인상을 다루는 영역이다.  
직접적 고백, 거리감, 미묘한 반응 차이, 가까워진 뒤의 태도 변화처럼 관계에서 체감되는 결을 자유롭게 해석할 수 있다.  
설명은 방향만 정할 뿐이며, 특정 장면 문장을 제공하지 않는다. 결과 문장은 매번 새롭게 만들어야 한다.
또한 MZ 스타일로 재미있고 위트있게 표현해야한다.

**oneLineVerdict 작성 (연애운 전용)**  
- 반드시 분야 키워드 **"연애운" 또는 "연애" 또는 "관계"** 중 1개 포함
- 아래 공식을 따른다:  
  [연애 맥락 A]보다, [연애 맥락 B]인 [연애운/관계 표현].
- 문장은 새롭게 창작해야 하며, 익숙한 연애 조언 문구처럼 보이지 않도록 한다.
- ❌ 아래 문장을 그대로 쓰면 실패:  
  "불꽃형 연애보다, 오래 가는 신뢰형 연애운."
- ❌ 재물운·직장운 어휘 금지:  
  "재물운", "벼락부자형", "적금왕", "직장운", "일잘러", "실속형"

**characterNickname (연애 전용)**  
- 반드시 **~타입** 어미 사용
- 관계에서의 태도나 감정 결을 드러내는 **2~3어 조합**으로 새롭게 작명
- 금지 예시와 유사한 표현 재사용 금지
- ❌ "무심한 듯 다정한 타입" 그대로 사용 금지

**nicknameSubtext**  
- 관계, 감정, 거리, 온도, 시선, 결, 리듬 같은 은유를 활용할 수 있다
- 시간·축적 은유는 과하게 반복하지 않는다

**body 작성 규칙**  
- 관계 안에서 느껴지는 태도, 반응, 애정 방식, 미묘한 인상을 중심으로 쓴다
- 분석처럼 딱딱하지 않게, 관찰적이고 사람 냄새 나는 문장으로 쓴다
- 감정을 단정하기보다 “이런 식으로 읽히기 쉽다”, “이런 결로 보이기 쉽다” 같은 인상 해석이 좋다
