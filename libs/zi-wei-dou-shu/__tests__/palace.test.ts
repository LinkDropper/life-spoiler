...
describe("궁 계산", () => {
  describe("calculateMingGong", () => {
    ...
    it("12월생 해시는 인궁(2)이다", () => {
      // (2 + 12 - 1 - 11 + 12) % 12 = 2
      expect(calculateMingGong(12, 11)).toBe(2);
    });
  });
  ...
  describe("calculateShenGong", () => {
    ...
    it("3월생 사시 신궁은 유궁(9)이다", () => {
      // (2 + 3 - 1 + 5) % 12 = 9
      expect(calculateShenGong(3, 5)).toBe(9);
    });
  });
  ...
  describe("getPalaceStem", () => {
    ...
    it("갑년 자궁의 천간은 병(2)이다", () => {
      // 갑년 인궁 천간=병(2), 자궁 offset=(0-2+12)%12=10, 천간=(2+10)%10=2
      expect(getPalaceStem(0 as StemIndex, 0 as BranchIndex)).toBe(2);
    });
