import { getStarBrightness, isPositiveBrightness } from "../calculators";
import type {
  Brightness,
  FortuneSection,
  Palace,
...
const getMainStarInterpretation = (
  ...
  let interpretationTable: Record<
    string,
...
const summarizeMinorStars = (palace: Palace): string => {
  ...
  for (const star of palace.minorStars) {
    ...
    if (["좌보", "우필", "천괴", "천월", "문창", "문곡", "천마"].includes(star.name)) {
  ...
  if (positiveStars.length > 0) {
    parts.push(`길성 ${positiveStars.join(", ")}이(가) 함께하여 운세를 돕습니다.`);
  }

  if (negativeStars.length > 0) {
    parts.push(`살성 ${negativeStars.join(", ")}이(가) 있어 주의가 필요합니다.`);
  }
...
export const generateHeadline = (chart: ZiweiChart): string => {
  ...
  if (starTemplates) {
    return starTemplates[brightestStar.brightness] || `${brightestStar.name}의 기운을 받은 인생`;
  }
...
export const generatePreviewDescription = (chart: ZiweiChart): string => {
  ...
  if (analysis.mainStarInterpretations.length > 0) {
    ...
    const firstInterpretation = analysis.mainStarInterpretations[0];
