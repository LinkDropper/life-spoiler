...
**계정 귀속(2026-08-19 구현, 마이그레이션 미적용)**: Phase 1(토큰 발급, `OWNER_TOKEN_COOKIE`)에 이어 Phase 2(검증/귀속)를 구현. 마이그레이션 007에 `universes.user_id`(nullable, FK → users, `ON DELETE SET NULL`) 추가. 귀속 경로 두 가지: (1) 생성 시 로그인 상태면 `app/api/universe/route.ts`가 `createAuthClient().auth.getUser()`로 즉시 `createUniverse`에 넘김 (2) 비로그인 생성 후 로그인 시 `app/auth/callback/route.ts`가 `OWNER_TOKEN_COOKIE` 원문 토큰을 읽어 `claimUniverseByOwnerToken` 호출 — `repository.ts`의 `claimUniverseOwner`가 `WHERE owner_token_hash = X AND user_id IS NULL`로 이미 귀속된 우주를 덮어쓰지 않는다. 콜백에서는 반드시 try/catch로 감싸 로그인 리다이렉트를 막지 않는다(auth 실패보다 우선순위 낮은 부가 기능). `supabase-write.ts`의 `WriteFilterBuilder`에 `is()` 메서드를 추가해야 했음(기존엔 `eq`/`or`만 있었음, IS NULL 조건에 필요).
